...
Chess board evaluation utilities.

This module provides heuristics for scoring chess positions and moves
using material values and piece-square tables. It supports:
  • Assigning values to pieces and positions
  • Evaluating captures and promotions
  • Evaluating entire board positions
  • Determining whether a position is in the endgame phase
"""
...
# Piece-square tables: positional bonuses for each square.
# White tables are listed top-to-bottom; Black tables are reversed.
...
def move_value(board: chess.Board, move: chess.Move, endgame: bool) -> float:
    ...
    Considers:
    - Promotions (very high value)
    - Captures (weaker piece taking stronger is good, reverse is bad)
    - Positional changes from piece-square tables

    Returns a signed value (positive for White's perspective, negative for Black).
    """
...
def evaluate_capture(board: chess.Board, move: chess.Move) -> float:
    """
    Evaluate the material trade of a capture.
    Returns the value of the captured piece minus the value of the capturing piece.
    Special handling is included for en passant captures.
    """
...
def evaluate_piece(piece: chess.Piece, square: chess.Square, end_game: bool) -> int:
    """
    Evaluate a piece on a given square using piece-square tables.
    Selects the correct table depending on:
    - Piece type
    - Piece color
    - Whether the game is in the endgame phase (for kings)
    """
...
def evaluate_board(board: chess.Board) -> float:
    ...
    Magnitude represents size of the advantage.
    Returns:
    Positive value if White is favored,
    Negative value if Black is favored.
    """
    total = 0
...
def check_end_game(board: chess.Board) -> bool:
    ...
    Uses Michniewski’s heuristic:
    - Both sides have no queens, or
    - Any side with a queen has at most one minor piece (bishop or knight).
    """
    queens = 0
    minors = 0
